...
Dim i As Long
    ...
    Call TcpInit(GAME_SERVER_IP, LOGIN_SERVER_PORT)
    Call InitMessages
    ' Reset values
    Ping = -1
...
Public Sub LoginGame(ByVal Login As String, ByVal uniqueKey As String)
    TcpInit GAME_SERVER_IP, GAME_SERVER_PORT
    
    If ConnectToServer Then
        Call SetStatus("Sending login information.")
        Call SendLoginGame(Login, uniqueKey)
        Exit Sub
    Else
        ShowWindow GetWindowIndex("winLogin")
        Dialogue "Connection Problem", "Cannot connect to login server.", "Please try again later.", TypeALERT
...
Public Sub SendLoginGame(ByVal Login As String, ByVal uniqueKey As String)
    Dim Buffer As clsBuffer
    Set Buffer = New clsBuffer
    Buffer.WriteLong clogin
    Buffer.WriteString Login
    Buffer.WriteString uniqueKey
