...
Public Sub InitMessages()
    ...
    HandleDataSub(SLoginToken) = GetAddress(AddressOf HandleLoginToken)
End Sub
...
Sub HandleAlertMsg(ByVal index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    Dim Buffer As clsBuffer, dialogue_index As Long, menuReset As Long, kick As Long, uniqueKey As String
...
Private Sub HandlePlayerChars(ByVal index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    Dim Buffer As clsBuffer, i As Long, winNum As Long, conNum As Long, isSlotEmpty(1 To MAX_CHARS) As Boolean, SlotReceived(1 To MAX_CHARS) As Boolean, x As Long
    Dim slotNum As Long, count As Long
    ...
    If count > 0 Then
        For i = 1 To count
            ...
            If slotNum > 0 Then
                SlotReceived(slotNum) = True

                CharName(slotNum) = Trim$(Buffer.ReadString)
                CharSprite(slotNum) = Buffer.ReadLong
                CharAccess(slotNum) = Buffer.ReadLong
                CharClass(slotNum) = Buffer.ReadLong
                If Len(Trim$(CharName(slotNum))) <= 0 Then SlotReceived(slotNum) = False
            End If
        Next i
    End If

    For i = 1 To MAX_CHARS
        If SlotReceived(i) = False Then
            CharName(i) = vbNullString
            CharSprite(i) = 0
            CharAccess(i) = 0
            CharClass(i) = 0
            ' set as empty or not
...
Private Sub HandleLoginToken(ByVal index As Long, ByRef data() As Byte, ByVal StartAddr As Long, ByVal ExtraVar As Long)
    Dim Buffer As clsBuffer
