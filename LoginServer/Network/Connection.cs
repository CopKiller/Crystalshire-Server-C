...
namespace LoginServer.Network
{
    public sealed class Connection : IConnection
    {
        ...
        public void ReceiveData()
        {
            ...
            Global.WriteLog(LogType.Debug, $"Receive -> {IpAddress} {GetType().Name} Buffer Size: {msg.Length()}", ConsoleColor.Black);
            ...
            while (pLength > 0 && pLength <= msg.Length() - 4)
            {
                if (pLength <= msg.Length() - 4)
                {
                    ...
                    if (OpCode.RecvPacket.ContainsKey(header))
                    {
                        ...
                        Global.WriteLog(LogType.System, $"Recebido: {Enum.GetName(typeof(Packet), header)}", ConsoleColor.Black);
                    }
                    else
                    {
